...
When a task fires, the scheduler reuses `crate::agent::create_worktree_and_spawn_agent` — the same helper used by the manual `Opt+E` flow — so worktree creation, branch selection and the `auto_exit` (Stop hook) wiring all behave identically. The `inactive`→`active` transition is performed **inside the same hub-lock window as the agent insert** (via the `scheduled_task_id: Some(...)` field on `CreateWorktreeParams`); this closes a race where a fast-exiting agent's PTY reader cleanup could otherwise run before the task left `inactive`, marking zero rows complete and leaving the task stuck `active` until the next hub restart flipped it to `aborted`.
