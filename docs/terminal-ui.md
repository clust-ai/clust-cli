...
- **Global section (always shown):** `q / Esc×2`, `Q`, `Ctrl+C`, `Tab`, `Shift+Tab`, `?`, `F2`, `Alt+R`, `Alt+D`, `Alt+F`, `Alt+N`, `Cmd+1`, `Cmd+2`.
...
The `Opt+R` / `Alt+R` and `Opt+N` / `Alt+N` hints are shown in the status bar (platform-aware).
