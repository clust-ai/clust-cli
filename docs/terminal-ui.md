...
- `TerminalPanel` struct manages the terminal state: ID, `TerminalEmulator`, command channel, exited flag, scroll offset, per-panel event receiver, and the background task handle
- `TerminalOutputEvent` enum carries output, exited, and connection-lost events from the background task to the UI thread via an `mpsc` channel; each `TerminalPanel` owns its own receiver so output keeps flowing into its `vterm` whether or not focus mode is currently displaying it
...
- `FocusModeState::drain_terminal_events()` is called each frame in the main event loop alongside `drain_output_events()` and `drain_diff_events()`, and forwards to the active panel's `drain_events()`. `OverviewState::drain_cached_terminal_events()` is called every tick as well so cached focus-mode panels accumulate scrollback while focus mode is closed
...
- On focus mode exit, the terminal panel is detached from `FocusModeState` (via `FocusModeState::detach()`, which closes the agent panel and auxiliary tasks but leaves the terminal panel intact) and stashed on `OverviewState::agent_terminals: HashMap<String, TerminalPanel>` keyed by `agent_id`. No `DetachTerminal` is sent and the shell process keeps running. On the next focus-mode entry for the same agent, `OverviewState::take_agent_terminal` removes the cache entry and `FocusModeState::open_agent` is called with `existing_terminal: Some(panel)`, which routes through `install_existing_terminal` to re-attach the cached panel and resize it to the current focus dimensions instead of spawning a new shell. App-level shutdown still uses `FocusModeState::shutdown()` (which calls `close_panel()`) to fully tear down the terminal
- Cached terminal panels are pruned by `OverviewState::sync_agents`: any cache entry whose `agent_id` is no longer present in the agent list has its background task aborted and is dropped (covers both the in-focus-when-agent-dies path and the close-focus-then-agent-dies path)
