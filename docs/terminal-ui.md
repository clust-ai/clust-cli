...
- The **terminal area** below the header renders the agent's PTY output using a `vt100`-backed terminal emulator (`TerminalEmulator`) with full ANSI support (cursor movement, SGR colors/styles, erase operations, scroll regions, line wrapping, alternate screen buffer). The terminal emulator gets the inner width (total panel width minus 2 border columns). **Batch agent panels skip terminal content rendering entirely** -- when `batch_info` is `Some`, only the header row is shown. The panel header with agent ID, status, repo, and branch info is sufficient context for batch overview; full terminal output is available via focus mode.
...
- `tasks` (array, required) -- list of task objects with `branch` (string, required), `prompt` (string, required), `use_prefix` (bool, default true), and `use_suffix` (bool, default true)
