...
│└──── Shift+↓ focus──┘│└────────────────┘│└─────────┘│
├──────────────────────┴──────────────────┴───────────┤
│ ● connected  Shift+↓ enter terminal  ...    v0.0.26 │
└─────────────────────────────────────────────────────┘
```
...
| Options Bar | `Alt+X` (`Opt+X` on macOS) | Disable auto-exit on the most recently focused panel (no-op if it isn't on). Uses Alt rather than Shift because Shift+X would collide with typing a capital `X`. |
...
| Terminal | `Alt+X` (`Opt+X` on macOS) | Disable auto-exit on the focused panel (no-op if it isn't on). Uses Alt rather than Shift to avoid colliding with typing capital `X` into the agent terminal. |
...
```
● connected  q to quit  Q to quit and stop hub  ↑↓←→ navigate  Shift+←→ panels                           v0.0.26
```
...
| Version | Right-aligned, e.g. `v0.0.26` |
...
│                               │└────────────────────┘│
├─────────────────────────────────────────────────────┤
│ ● connected  Shift+←/→ switch panel  ...     v0.0.26│
└─────────────────────────────────────────────────────┘
```
...
**Typing into Active panels.** When the focused task is `Active`, regular keys (and pasted text wrapped in bracketed-paste markers) are forwarded directly to that agent's PTY — no need to drop into focus mode for quick interaction. Reserved keys: `Shift+←/→` (switch panel), `Shift+↑` (return focus to the bar), `Shift+↓` (focus mode), `PgUp`/`PgDn` (scroll the panel's scrollback). Everything else, including `Esc`, `Ctrl+C`, `q`, `Tab`, `Shift+C`, etc., is forwarded to the agent. To use `q`/`Tab`/`Shift+C` as TUI commands, navigate to a non-Active task first with `Shift+←/→`. Mirrors Overview's terminal-focus key flow.

**Top bar.** Mirrors the Overview options bar. On the left, one chip per repo that currently has a scheduled task: a coloured `●` plus the repo name in primary text. A vertical-bar separator follows, then one branch indicator per task. Branch indicators are coloured by repo: tasks visible on screen render in inverse video (repo-coloured background, primary-text foreground); tasks scrolled off render in repo-coloured text on the bar background; tasks belonging to a collapsed repo render in `R_TEXT_DISABLED` on the bar background. Each branch chip is **clickable** — clicking one focuses that task, scrolls the panel grid until it is visible, and un-collapses its repo if it had been filtered out. The bar background brightens to `R_BG_OVERLAY` when the bar itself is focused (versus `R_BG_RAISED` when a task is focused) so the user can tell where keystrokes will land.

**Top bar focus and filtering.** The bar takes focus on first entry into the tab and stays there across periodic re-syncs — no auto-jump onto a task — so the user always lands on the repository filter (or whatever was selected last) rather than a randomly-chosen panel. The bar is also focused after `Shift+↑` from a task, or when the task list is empty. From the bar:
- `←` / `→` cycle the cursor through the repo chips (the cursor position renders with `R_BG_ACTIVE` background).
- `Enter` / `Space` toggle the chip under the cursor — collapsed repos dim their dot and name and hide their tasks from the panel grid (and from `Shift+←/→` navigation), but their chip and branch indicators stay in the bar so the user can re-enable them.
- `Shift+↓` moves focus into the last task the user had focused (or the first visible task on the very first entry). Mirrors Overview's `last_terminal_idx` restore.
Clicking a repo chip toggles its collapsed state without moving keyboard focus. If the focused task ends up in a now-collapsed repo, focus jumps to the next visible task (or back to the bar if none remain).
...
**Dep picker contents.** The `Depend` step lists every existing scheduled task **and** every currently-running Opt+E worktree agent (with a known repo + branch and no shadow task linked yet). Task rows display `repo / branch [STATUS]` where `STATUS` is one of:

- `FUTURE` — Inactive task that will create a new branch (the branch doesn't exist on disk yet — chain off this row to wait for that branch to be created)
- `PENDING` — Inactive task that reuses an existing branch
- `RUNNING` — Active task currently driving an agent
- `DONE` — Completed task
- `ABORTED` — Aborted upstream (selecting this would block the new task indefinitely)

Agent rows display `repo / branch [AGENT AUTO-EXIT]` or `[AGENT no-auto-exit]` so you can see at a glance whether the upstream agent will reach `Complete` on its own. Rows are sorted Future → Pending → Running → Agents → Done → Aborted when the input is empty so the most chain-relevant entries float to the top. Selecting an agent as a dep triggers the hub to insert a shadow `scheduled_tasks` row (`status=active`, `agent_id=<the running agent>`, `schedule_kind=unscheduled`) before persisting the new task — that's the only path by which an Opt+E agent shows up on the Schedule tab. When the agent exits, the shadow row flips to `Complete` via the existing PTY-reader hook, unblocking downstream tasks.

**Future branch as base.** Under `Depend`, the SelectBranch step also offers *future* branches — branches that don't exist locally yet but will be created by an Inactive scheduled task in the same repo. They render as `<branch>  future — created by scheduled task`. Picking one records the upstream task id, pre-checks it in the dep picker, and re-injects it into `depends_on_ids` at submission so `git worktree add <wt> <future-branch>` only runs after the upstream creates that branch. Future branches are not offered for `Schedule`/`Unscheduled` kinds (no dep mechanism would enforce ordering there).
